...
| Arabert                                  | None                               | 0.7032              | 0.7230               | [model.zip](https://bit.ly/3g2Zg98) |
| bert-base-multilingual-cased             | Farasa Segmentation                | 0.6766              | 0.6970               | [model.zip](https://bit.ly/2P6ujF2) |
| bert-base-multilingual-cased             | None                               | 0.6529              | 0.6706               | [model.zip](https://bit.ly/2WYE0dh) |

If you are considering a model that was preprocessed, use the following script to preprocess your input text. The example can be found in [Arabic STS Experiment](examples/arabic_sts/run_experiment.py)
...
def preprocess(text, do_farasa_tokenization=True, farasa=None, use_farasapy=False):
	...
	"""
    text = str(text)
    processing_text = araby.strip_tashkeel(text)
    processing_text = re.sub(r"\d+\/[ء-ي]+\/\d+\]", "", processing_text)
    processing_text = re.sub("ـ", "", processing_text)
    processing_text = re.sub("[«»]", ' " ', processing_text)
    # replace the [رابط] token with space if you want to clean links
    processing_text = re.sub(regex_url_step1, "[رابط]", processing_text)
    processing_text = re.sub(regex_url_step2, "[رابط]", processing_text)
    processing_text = re.sub(regex_url, "[رابط]", processing_text)
    processing_text = re.sub(regex_email, "[بريد]", processing_text)
    processing_text = re.sub(regex_mention, "[مستخدم]", processing_text)
    processing_text = re.sub("…", r"\.", processing_text).strip()
    processing_text = remove_redundant_punct(processing_text)

    processing_text = re.sub(
        r"\[ رابط \]|\[ رابط\]|\[رابط \]", " [رابط] ", processing_text
    )
    processing_text = re.sub(
        r"\[ بريد \]|\[ بريد\]|\[بريد \]", " [بريد] ", processing_text
    )
    processing_text = re.sub(
        r"\[ مستخدم \]|\[ مستخدم\]|\[مستخدم \]", " [مستخدم] ", processing_text
    )

    processing_text = remove_elongation(processing_text)
    if do_farasa_tokenization and farasa is not None:
        processing_text = tokenize_arabic_words_farasa(
            processing_text, farasa, use_farasapy
        )
    return processing_text.strip()


def segment(text):
    return preprocess(text, do_farasa_tokenization=True, farasa=farasa_segmenter, use_farasapy=True)
```